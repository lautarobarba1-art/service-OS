    ...
    <div className="dashboard-chart-grid">
      <section className="metric-panel chart-panel">
        ...
        <div className="chart-container" style={{ width: "100%", height: "260px" }}>
          {isMounted ? (
            ...
            <div className="empty-chart" aria-hidden="true">Cargando gráfico…</div>
          )}
        </div>
      </section>
      <section className="metric-panel chart-panel status-chart-panel">
        ...
        <div className="status-chart-body">
          <div className="pie-container" style={{ width: "100%", height: "180px" }}>
            {hasStatuses ? (
              isMounted ? (
                <ResponsiveContainer height="100%" minHeight={0} minWidth={0} width="100%">
                  <PieChart>
                    <Pie data={statusSeries} dataKey="value" innerRadius={48} nameKey="label" outerRadius={72} paddingAngle={2}>
                      {statusSeries.map((item) => <Cell fill={item.color} key={item.label} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
                <span className="empty-chart">Cargando gráfico…</span>
              )
            ) : (
              <span className="empty-chart">Sin datos</span>
            )}
          </div>
